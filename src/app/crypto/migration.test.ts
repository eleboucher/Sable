...
const commands = vi.hoisted(() => ({
  engineImportRoomKeys:
    vi.fn<
      (params: { keysJson: string }) => Promise<{ imported_count: number; total_count: number }>
    >(),
}));
...
describe('handOverRoomKeys', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    commands.engineImportRoomKeys.mockImplementation(async ({ keysJson }) => {
      const count = (JSON.parse(keysJson) as unknown[]).length;
      return { imported_count: count, total_count: count };
    });
  });
  ...
  it('hands the whole export over in one batch when it fits', async () => {
    ...
    expect(JSON.parse(commands.engineImportRoomKeys.mock.calls[0]![0].keysJson)).toEqual(keys);
  });
  ...
  it('splits a large export so no single IPC payload holds every session', async () => {
    ...
    const sizes = commands.engineImportRoomKeys.mock.calls.map(
      ([params]) => (JSON.parse(params.keysJson) as unknown[]).length
    );
    expect(sizes).toEqual([500, 500, 200]);
  });

  it('reports sessions the engine rejected as duplicates', async () => {
    commands.engineImportRoomKeys.mockResolvedValue({ imported_count: 1, total_count: 3 });
    const source = {
  ...
  it('does no IPC at all when the old device has no keys', async () => {
    ...
    expect(commands.engineImportRoomKeys).not.toHaveBeenCalled();
  });
  ...
  it('stops on a failed batch rather than silently dropping the rest', async () => {
    commands.engineImportRoomKeys
      .mockResolvedValueOnce({ imported_count: 1, total_count: 1 })
    ...
    expect(commands.engineImportRoomKeys).toHaveBeenCalledTimes(2);
  });
